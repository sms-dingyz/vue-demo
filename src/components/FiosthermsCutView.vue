  ...
  export default {
    ...
    drawAxis() {
		...
		this.topLayertemperatureData.forEach((item,index)=>{	
			...
			// 	console.log("________dyzdyz index:" + index+ " ctx.lineWidth:"+ ctx.lineWidth)
			// 	console.log("--- dyzdyz index:" + index +  "  startx:"+item.startx +" starty:"+ item.starty
	...
	getElementInformation(data ,index){
		...
		ctx.lineWidth = parseInt(spacingY); 
		...
		for (var i = 0; i < data.length  ; i++) {
			...
			// endy = originY -  index*parseFloat(spacingX)
	...
	convertTemperatrueToColor(){
		...
		temperatureData.forEach((item,index)=>{ 
					...
					row = parseInt((index/120))
					var rgb = this.tempToColor(item,800,1800,"extended")
